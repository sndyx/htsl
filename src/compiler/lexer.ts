...
export class Lexer {
    ...
    advanceToken(): Token {
        ...
        if (c == "+") {
            ...
            } else return token("bin_op", singleSpan, { op: "plus" });
        }
        if (c == "-") {
            if (this.peek(0) == "=") {
                this.next();
                return token("bin_op_eq", span(lo, lo + 1), { op: "minus" });
            } else return token("bin_op", singleSpan, { op: "minus" });
        }
        if (c == "*") {
            if (this.peek(0) == "*") {
                this.next();
            ...
            } else return token("bin_op", singleSpan, { op: "star" });
        }
        if (c == "/") {
            ...
            } else return token("bin_op", span(lo, this.pos), { op: "slash" });
        }
        ...
        if (c == "=") {
            ...
            } else return token("cmp_op", singleSpan, { op: "equal" });
        }
        if (c == "<") {
            if (this.peek(0) == "=") {
                this.next();
                return token("cmp_op_eq", span(lo, lo + 1), { op: "less_than" });
            } else return token("cmp_op", singleSpan, { op: "less_than" });
        }
        if (c == ">") {
            if (this.peek(0) == "=") {
                this.next();
                return token("cmp_op_eq", span(lo, lo + 1), { op: "greater_than" });
            } else return token("cmp_op", singleSpan, { op: "greater_than" });
        }
        ...
        if (/[0-9]/.test(c)) {
            ...
            } else return token("i64", span(lo, this.pos), { value });
        }
        ...
        throw Error(`unknown token "${c}"`) // Eventually turn this into a diagnostic
    }
